...
describe('Weth', function () {
  ...
  describe('Mint', () => {
    it('to the address successfully', async function () {
      ...
      await expect(tx).to.emit(token, "Transfer").withArgs(account1.address, wrong_address, 100);
    ...
    it('revert transaction, due to wrong address', async function () {
      await expect(token.connect(owner).mint(wrong_address, 100)).to.be.revertedWith("ERC20: mint to the zero address");
    });
    it('revert transaction, due to done not by the owner', async function () {
      // let tx = token.connect(account1).mint(account2.address, 100);
  ...
  describe('Burn', () => {
    it('a certain amount of tokens', async function () {
      ...
      await expect(tx).to.emit(token, "Transfer").withArgs(account1.address, wrong_address, 1);
      const new_balance = await token.balanceOf(account1.address);
      const new_total_supply = await token.totalSupply();
    ...
    describe('revert transaction, ', () => {
      it('due to too low balance of certain address', async function () {
        ...
        let tx = token.connect(owner).burn(owner.address, balance + 1);
  ...
  describe('Allowance', () => {
    ...
    describe('revert', async function () {
      it('increaseAllowance transaction', async () => {
        await expect(
          token.connect(owner).increaseAllowance(wrong_address, 100)
  ...
  describe("Transfer transaction", function () {
    ...
    describe('is reverted', () => {
      it('due to wrong address', async () => {
        await expect(
          token.connect(owner).transfer(wrong_address, 100)
  ...
  describe('Approve', () => {
    ...
    it("revert transaction", async function () {
      await expect(
        token.connect(owner).approve(wrong_address, 1000)
      ).to.be.revertedWith("ERC20: zero address");
    });
  })
})
